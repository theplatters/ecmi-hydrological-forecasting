...
\resizebox{1\columnwidth}{!}{\includegraphics{ecmi-logo(1).png}}
...
\begin{figure}[h]
  \resizebox{0.5\columnwidth}{!}{\includegraphics{map_everything4(1).jpg}}
...
%from the input data we feed to the ML model. 
...
We decided to fill the missing data of station $A$ with the data of station $B$
...
\qquad calculate and store the Pearson correlation of $A$ and $B$

\qquad if the p-value is greater than a threshold, then disregard $B$
...
Calculate the correlation $c_{i, X}$ of the water level between station "Szeged" at day
